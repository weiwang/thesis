...
As it is well known, although the procedure for Gaussian Processes inference is
quite straightforward, the main difficulty lies in the matrix inversions
required for both estimating hyperparameters and predicting at new points, which
involves $\mathcal{O}(N^3)$ time complexity with $N$ being the number of
observations. Data sets that have more than 1,000 observations are already
prohibitively expensive for computation. In those cases, a number of
approximation methods such as low-rank approximations of the Gram kernel matrix,
known as Nystrom method [@williams2001using], and judicious selections of subset
of observations [@banerjee2008gaussian] are often recommended.


## Gaussian Process with Hierarchical Structure ##

Gaussian Process can be extended to handle group structure. A common approach
from machine learning perspective is to pose this question as a multi-task
learning problem [@yu2005learning; @bonilla2008multi], in which the objective
...
become even more acute, as the sample size effectively mutiples by the number of
...
\noindent Assuming separability can significantly reduce the dimentionality of
...
the scenario of "complete design", meaning every combination of predictors
values have been observed. This assumption is reasonable for areas such as
computer experiments and robotics, where experiments can be carried out at
pre-specified values of X's; but it is quite impractical in fields such as
education and public health, where researchers' control over data collections is
much limited.

### Building the Group Structure into Kernels ###

However, it is relatively straightforward to directly code the group structure
into kernels. Take the most popular kernel, square exponential kernels, for
example, we can add discrete group indicator terms by using delta metrics, i.e.,
0 if two observations are from the same group, and 1 otherwise. Further, it is
often important to add group-level predictors in hierarchical models
[@gelman2006data], since it can account for some of the differences that cannot
be explained by group structure.

\begin{equation*}
\kappa(x_i, x_j)=\sigma^2\exp{-\frac{1}{2}\sum_{k=1}^d \frac{(x_{ik}-x_{jk})^2}{l_k^2}}
\end{equation*}

\noindent In square exponential kernels, the length scale $l_k$ govenst he correlation scale in input dimension $k$ and teh magnatitude $\sigma^2$ controls the overall variability of the process. Thus the magnitudes of the length scale $l_k$ are often used for feature selection; the larger the length scale, the more important the corresponding feature.
...
project, which began in 1985 to study the effect of early grades class size on
student achievement in Tennessee. The study is a state-wide randomized
...
\url{http://www.heros-inc.org/star.htm}, with 11,598 observations on 47
variables, including information on the student, test scores and treatment
assignment over the intervention years, information of the teacher, and school
id et al. Due to its richness, STAR project data can be investigated in many
different facets. For the sake of simplicity, we choose to look at just one
outcome, standardized math test score, in one intervention year, the 1st
grade. Thus we can focus on the meta-analytic part of the data, without being
distracted by the longitudinal aspect of the data, which is a nuisance for our
discussion. Further, as mentioned previously, data sets with thousands of
observations pose prohibitive computational burden on Gaussian processes. So I
select a subset of the data set, including only 8 biggest schools for the first
grade.
...
receving free lunch, which gives us a school-level predictor. As for types of
treatment, as I mentioned, there are three types of treatment, samll-size
classroom, regular-size classroom and regular with a paid teaching aid. Instead
of combining regular and regular with an aid, an appoach adopted by most of
previous literature, I treat them as separate interventions. All of the above
predictors are fed into a square exponentional kernel, with a delta metric for
